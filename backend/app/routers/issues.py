...
from ..schemas import IssueCreate, IssueUpdate, IssueResponse, PaginatedResponse, SeverityEnum, StatusEnum
...
def create_issue(payload: IssueCreate, db:Session=Depends(get_db), current_user: User=Depends(get_current_user)):
    ...
    db.add(issue)
    db.commit()
    issue=db.query(Issue).options(joinedload(Issue.creator), joinedload(Issue.assignee)).filter(Issue.id==issue.id).first()
    return issue

@router.get("/", response_model=PaginatedResponse[IssueResponse])
def list_issues(status: Optional[StatusEnum]=Query(None), severity: Optional[SeverityEnum]=Query(None),search: Optional[str]=Query(None),page: int=Query(1, ge=1), size:int=Query(10, ge=1, le=100), db:Session=Depends(get_db), current_user: User=Depends(get_current_user)):
    ...
    return {"items": items, "total": total, "page": page, "size": size, "pages": pages, "stats": stats_data}
...
def update_issue(issue_id: uuid.UUID, payload: IssueUpdate, db:Session=Depends(get_db), current_user: User=Depends(get_current_user)):
    ...
    db.commit()
    return db.query(Issue).options(joinedload(Issue.creator), joinedload(Issue.assignee)).filter(Issue.id == issue_id).first()
