...
from ..schemas import CommentCreate, CommentResponse, CommentUpdate, PaginatedComments
...
def list_comments(
    ...
    items = (
        comments.filter(Comment.issue_id == issue_id)
...
async def create_comment(
    ...
    await manager.broadcast(
        f"issue:{issue_id}", {"type": "comment_created", "issue_id": str(issue_id)}
    )
    return comment
...
async def update_comment(
    ...
    comment = (
        db.query(Comment).filter(Comment.id == comment_id).filter(Issue.id == issue_id).first()
    )
    if not comment:
    ...
    return (
        db.query(Comment)
        .options(joinedload(Comment.commenter), joinedload(Comment.issue))
